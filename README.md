...
This Keras implementation for image inpainting is based off following papers:

* Liu *et al.*,  **Image Inpainting for Irregular Holes Using Partial Convolutions**, https://arxiv.org/abs/1804.07723
* Yu *et al.*,  **Free-Form Image Inpainting with Gated Convolution**, https://arxiv.org/abs/1806.03589
...
If you want to dig into the code, the primary implementations can be found in following files:

* `libs/pconv_layer.py` implements the `PConv2D` layer introduced in Liu *et al.* paper
* `libs/gconv_layer.py` implements the `GConv2D` layer introduced in Yu *et al.* paper
* `libs/unet_model.py` implements the `UNet`-like architecture introduced in Liu *et al.*

Beyond this I've set up five jupyter notebooks, which details the several steps I went through while implementing the network, namely:

* **Step 1:** Creating random irregular masks<br />
* **Step 2:** Implementing and testing the implementation of the `PConv2D`/`GConv2D` layers<br />
* **Step 3:** Implementing and testing the UNet architecture with `PConv2D`/`GConv2D` layers<br />
* **Step 4:** Training & testing the models on ImageNet<br />
* **Step 5:** Simple extension towards predicting arbitrary image sizes through image chunking
